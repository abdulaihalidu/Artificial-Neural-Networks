...
float* normalizeData(float* Samples, int numSample, int inputDim) {
	...
	for (int i = 0; i <= (numSample * inputDim); (i += inputDim)) {
		Mx += Samples[i];
		My += Samples[i + 1];
		number++;
	}
	...
	for (int i = 0; i < numSample; i++) {
		...
		for (int n = 0; n < neuronCount; n++) {
			if (desired[n] != output[n]) {  // update weights of neuron(s) whose outputs were different from the expected output
				cnt++;
				for (int j = 0; j < inputDim; j++) {
					Weights[2 * n + j] += c * (desired[n] - output[n]) * Samples[i * inputDim + j];
					bias[n] += c * (desired[n] - output[n]);
				}
			}
		}
...
float multiDeltaTraining(float* Samples, float* Weights, float* targets, float* bias, int numSample, int inputDim, int numClass) {
	...
	for (int i = 0; i < numSample; i++) {
		...
		for (int k = 0; k < K; k++) {
			for (int j = 0; j < inputDim; j++) {
				net[k] += Weights[j] * Samples[i * inputDim + j];
			}
			net[k] += bias[k];

			output[k] = bipolar_continous_activation_fun(net[k]);
			fnetprime[k] = sDerivative(output[k]);
		...
		err /= K; // determine average error produced

		// update weights;
		for (int k = 0; k < K; k++) {
			for (int j = 0; j < inputDim; j++) {
				Weights[j] += c * (desired[k] - output[k]) * fnetprime[k] * Samples[i * inputDim + j];
